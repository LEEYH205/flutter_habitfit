#!/usr/bin/env python3
"""
AI-Hub 음식 분류 모델 서버 API (간단 버전)
MMDetection 의존성 없이 기본 API 기능만 제공
"""
...
def load_class_names() -> List[str]:
    """AI-Hub 800개 한국 음식 클래스 목록을 반환합니다."""
    class_names_list = [
        ...
        '주꾸미삽겹살볶음', '크림새우', '난자완스', '옥수수시리얼', '감자옹심이', '햄샌드위치', '먹태', '루이보스차', '쟁반짜장', '양지쌀국수', 
        '로제파스타', '어묵튀김', '참치샌드위치', '크림치즈베이글', '맘모스빵', '중화볶음면', '초계탕', '오트밀시리얼', '감바스', '돈부리덮밥', 
        '통밀식빵', '장어튀김', '브라우니', '치즈라볶이', '크리스피치킨(뼈)', '빙떡', '유부우동', '아이스코코넛라떼', '닭가슴살스테이크', '망고주스', 
        '모히또', '메론빵', '비빔밀면', '닭윙&닭다리간장치킨', '추어전', '불닭마요컵밥', '초코시리얼', '칠리새우', '감자빵', '나쵸', 
        '깐쇼새우', '마라롱샤', '새우만두', '감자떡', '에그샌드위치', '새우김밥', '초코크로플', '마라탕', '탄두리치킨', '푸팟퐁커리', 
        '게살볶음밥', '케이준샐러드', '똠얌꿍', '날치알초밥', '초코케이크', '소보루빵', '찰순대', '체리콕에이드', '토마토계란볶음', '팥칼국수', 
        '미숫가루', '황태육개장', '마늘빵', '찹쌀꽈배기', '에비카츠', '오곡라떼', '인절미빙수', '단팥빵', '차돌떡볶이', '콜드브루', 
        '콜드브루라떼', '깜빠뉴', '얼그레이까눌레', '고구마라떼', '아이스초코라떼', '바나나쉐이크', '낙곱새', '소유라멘', '퀸아망', '아이스헤이즐넛라떼', 
        '아이스헤이즐넛아메리카노', '새우크림파스타', '레몬에이드', '호두타르트', '등갈비찜', '갈비만두', '갈비전골', '순살양념치킨', '차슈동', '분짜', 
        '교자만두', '치즈번', '초코밀크티', '초코마카롱', '소고기간', '카레고로케', '갈치구이', '누드김밥', '카츠나베', '야끼만두', 
        '카모마일차', '고등어구이', '핫바(어묵소시지)', '천엽', '아메리카노(HOT)', '갈비치킨', '딸기스무디', '치킨타코', '빨미까레', '굴비구이', 
        '치즈닭갈비', '매생이국', '시오라멘', '코코넛라떼(HOT)', '추어튀김', '치킨너겟', '꿍 팟 퐁 커리', '유자차', '망고에이드', '수박주스', 
        '자몽차', '청귤차', '블루레몬에이드', '냉우동', '아이스카라멜마끼아또', '메밀전', '아이스모카라떼', '히비스커스', '아이스초코모카라떼', '카페라떼(HOT)', 
        '도리뱅뱅이', '초코모카라떼(HOT)', '헤이즐넛아메리카노(HOT)', '유니짜장', '울면', '레몬아이스티', '녹차', '레몬차', '달고나라떼', '식혜', 
        '아이스토피넛라떼', '초코쉐이크', '밀크쉐이크', '낙지덮밥', '딸기빙수', '키위스무디', '토마토주스', '블랙밀크티', '생태찌개', '연유라떼', 
        '죠리퐁라떼', '칠리새우덮밥', '아샷추', '그린밀크티', '자몽에이드', '더치커피', '아이스카푸치노', '청포도에이드', '모카라떼(HOT)', '아포가토', 
        '티라미수라떼(HOT)', '돌솥비빔밥', '블루베리스무디', '바닐라라떼', '유자스무디', '코울슬로', '말차라떼', '아이스시나몬라떼', '아이스초코'
    ]
...
def predict_food_simple(image_data: bytes) -> Dict:
    """간단한 음식 분류를 수행합니다 (더미 데이터)."""
    try:
        # 이미지 전처리
        image = preprocess_image(image_data)
        
        # 더미 예측 결과 생성 (실제 모델이 없으므로)
        import random
        random.seed(hash(str(image.shape)) % 1000)  # 이미지 기반 시드
        
        # 랜덤하게 음식 선택
        selected_foods = random.sample(class_names, min(3, len(class_names)))
        
        predictions = []
        for i, food_name in enumerate(selected_foods):
            confidence = 0.9 - (i * 0.2)  # 신뢰도 감소
            predictions.append({
                'food_name': food_name,
                'confidence': confidence,
                'bbox': [100 + i*50, 100 + i*50, 200 + i*50, 200 + i*50]  # 더미 바운딩 박스
            })
        ...
        return {
            ...
            'note': 'This is a demo version with dummy predictions'
        }
...
def predict_weight(food_name: str) -> Dict:
    """음식 중량을 예측합니다."""
    try:
        # 음식별 평균 중량 (그램)
        average_weights = {
        ...
        return {
            ...
            'note': 'This is a demo version with estimated weights'
        }
...
def health_check():
    ...
    return jsonify({
        ...
        'version': 'demo',
        'models_loaded': True,
        'note': 'This is a demo version without actual AI models'
    })
...
def predict():
    ...
    try:
        ...
        # 음식 분류 수행
        result = predict_food_simple(image_data)
...
def predict_weight_api():
    ...
    try:
        ...
        # 중량 예측 수행
        result = predict_weight(food_name)
...
def get_classes():
    ...
    return jsonify({
        ...
        'note': 'This is a demo version with sample classes'
    })
...
def index():
    """API 문서"""
    return jsonify({
        'name': 'AI-Hub Food Classification API (Demo)',
        'version': '1.0.0-demo',
        'status': 'Demo version - No actual AI models loaded',
        'endpoints': {
            'GET /health': '서버 상태 확인',
...
if __name__ == '__main__':
    logger.info("AI-Hub 음식 분류 서버 (데모 버전) 시작 중...")
    
    # 클래스 이름 로드
    class_names = load_class_names()
    logger.info(f"✅ {len(class_names)}개 클래스 로딩 완료")
    logger.info("🚀 서버 시작: http://localhost:5000")
    logger.info("📝 참고: 이는 데모 버전입니다. 실제 AI 모델은 로드되지 않았습니다.")
    
    # Docker 환경에서는 Gunicorn 사용, 로컬에서는 Flask 개발 서버 사용
    if os.getenv('DOCKER_ENV'):
        # Docker 환경 - Gunicorn 사용
        import gunicorn.app.wsgiapp as wsgi
        sys.argv = ['gunicorn', '--bind', '0.0.0.0:5000', '--workers', '2', '--timeout', '120', 'app_simple:app']
        wsgi.run()
    else:

            ...
            Row(
              children: [
                Checkbox(
                    value: done,
                    onChanged: (v) => ref
                        .read(_habitDoneProvider.notifier)
                        .state = v ?? false),
                const Text('완료했어요'),
              ],
            ),
            ...
            ElevatedButton.icon(
              ...
              onPressed: () async {
                ...
                try {
                  // Firestore에 저장
                  await Fs.instance
                      .setHabitDone(uid, DateTime.now(), habitDone);

                  // 습관 체크 완료 시 알림 전송
                  if (habitDone) {
                    final prefs = await SharedPreferences.getInstance();
                    final habitRemindersEnabled =
                        prefs.getBool('habitRemindersEnabled') ?? true;
                    ...
                    if (goalAchievementEnabled) {
                      await LocalNotificationService.instance
                          .showGoalAchievementNotification('습관 체크', 1);

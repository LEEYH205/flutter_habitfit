            ...
            ElevatedButton.icon(
              ...
              onPressed: label == null
                  ...
                  : () async {
                      ...
                      await Fs.instance.addMeal(uid, DateTime.now(), label,
                          ref.read(_kcalProvider), null);
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('식사 기록 저장됨')));
                      }
                    },
            ),
            const SizedBox(height: 24),
            const Text('※ MVP: 자동 인식 대신 라벨 선택 → 평균 kcal 매핑',
